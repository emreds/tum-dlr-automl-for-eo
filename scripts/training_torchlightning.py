...
class LightningNetwork(pl.LightningModule):
    def __init__(self, params):
        ...
        # of average='micro'
        self.train_avg_macro_accuracy = MulticlassAccuracy(
            num_classes=self.num_class, average="macro"
        )
        self.train_avg_micro_accuracy = MulticlassAccuracy(
            num_classes=self.num_class, average="micro"
        )
        self.train_avg_all_accuracy = MulticlassAccuracy(
            num_classes=self.num_class, average=None
        )

        self.validation_avg_macro_accuracy = MulticlassAccuracy(
            num_classes=self.num_class, average="macro"
        )
        self.validation_avg_micro_accuracy = MulticlassAccuracy(
            num_classes=self.num_class, average="micro"
        )
        self.validation_avg_all_accuracy = MulticlassAccuracy(
            num_classes=self.num_class, average=None
        )
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        avg_macro_acc = self.train_avg_macro_accuracy(predictions, targets)
        avg_micro_acc = self.train_avg_micro_accuracy(predictions, targets)
        avg_all_acc = self.train_avg_all_accuracy(predictions, targets)

        self.log("train_loss", loss, on_step=False, on_epoch=True, sync_dist=True)
        self.log(
            "train_accuracy", accuracy, on_step=False, on_epoch=True, sync_dist=True
        )
        self.log(
            "train_avg_macro_accuracy", avg_macro_acc, on_step=False, on_epoch=True
        )
        self.log(
            "train_avg_micro_accuracy", avg_micro_acc, on_step=False, on_epoch=True
        )

        # after aggregating results across GPUs
        if self.global_rank == 0:
            logging.info(
                f"train_accuracy: {accuracy}, training train_avg_macro_accuracy: {avg_macro_acc}, train_avg_micro_accuracy: {avg_micro_acc}, train_loss: {loss}, train_avg_all_accuracy: {avg_all_acc}"
            )
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        avg_macro_acc = self.validation_avg_macro_accuracy(predictions, targets)
        avg_micro_acc = self.validation_avg_micro_accuracy(predictions, targets)
        avg_all_acc = self.validation_avg_all_accuracy(predictions, targets)
        ...
        self.log("validation_avg_macro_accuracy", avg_macro_acc, on_epoch=True)
        self.log("validation_avg_micro_accuracy", avg_micro_acc, on_epoch=True)

        # after aggregating results across GPUs
        if self.global_rank == 0:
            logging.info(
                f"validation_accuracy: {accuracy}, validation_avg_macro_accuracy: {avg_macro_acc}, validation_avg_micro_accuracy: {avg_micro_acc}, validation_loss: {loss}, validation_avg_all_accuracy: {avg_all_acc}"
            )
...
def get_args():
    ...
    parser.add_argument(
        "--data",
        default="/p/project/hai_nasb_eo/data",
        help="Path of the training data.",
    )
    parser.add_argument(
        "--result",
        default="/p/project/hai_nasb_eo/training/logs",
    ...
    parser.add_argument(
        ...
        default=64,
        type=int,
        help="Batch size should be divided by the number of gpus if ddp is enabled",
    )
    ...
    parser.add_argument(
        ...
        default=10e-5,
        type=float,
        help="learning rate should be scaled with the batch size \
    ...
    parser.add_argument(
        "--ddp", default=True, type=bool, help="Enable 1 node - multiple GPUs training"
    )
    parser.add_argument(
        "--gpus",
        default=4,
        type=int,
        help="Specify number of gpus used for training, given accelerator is gpu, can be >1 if ddp flag is enabled",
    )
    parser.add_argument(
        "--accelerator",
        default="gpu",
        type=str,
        help="Use different devices for training, e.g. gpu",
    )
    parser.add_argument(
        "--fast_dev_run",
        default=0,
        type=int,
        help="Test train/val/test pipeline by running a specific number of batches",
    )
...
def get_data_transforms():
    ...
    validation_data_std = [
        3.845561295747756958e-02,
        4.351711273193359375e-02,
        5.868862569332122803e-02,
        5.489562824368476868e-02,
        6.446107476949691772e-02,
        7.579671591520309448e-02,
        7.847409695386886597e-02,
        8.553111553192138672e-02,
        9.418702870607376099e-02,
...
if __name__ == "__main__":
    ...
    try:
        ...
        training_data = data_module.training_dataLoader(
            batch_size=batch_size, num_workers=num_workers
        )

        data_module.setup_validation_data(valid_transform)
        validation_data = data_module.validation_dataLoader(
            batch_size=batch_size, num_workers=num_workers
        )
